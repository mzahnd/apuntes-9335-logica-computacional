...
\begin{definicion}{Comparación de cardinales}{}
    ...
    \begin{enumerate}
        ...
        \item Decimos que $n = k$ $\iff$ existe $f: A \to B$ biyectiva, es
            decir, si $A \sim B$.
    \end{enumerate}
\end{definicion}


Veamos que no depende del representante elegido. 
\nota{\textit{Noni:} ``Buena definición.''}%
\begin{enumerate}
    \item Veamos que está bien definido. Es decir, que no depende del 
...
\begin{proof}
    Usando la Propiedad \ref{subsec:prop-card-lqeq}.
\end{proof}
...
\begin{proof}\phantom{.}
    ...
    \begin{align*}
        ...
        \implies&  a = c \wedge b=d \notamath{Por Teorema Fundamental de la 
        Aritmética (TFA)}
    \end{align*}
    
    Entonces, $g$ inyectiva 
    ...
    \item \begin{proof} \phantom{.}
            ...
            \begin{enumerate}
                ...
                \item $\exists k \in \mathbb{N}_{\geq 1} / 
                    ...
                    Por lo tanto, por ser $\tilde{h}$ biyectiva $\implies \#A = \#Im(h)$
                    y $Im(h) \subseteq I_k$

                $\underbrace{\implies}_{\text{Por el item 1}} Im(h)$ es finita
                $\implies$ $A$ es finito. ¡Absurdo!
            \end{enumerate}
...
\begin{proof}\phantom{.}
    ...
    $\exists \; x_1 \in X - \{ x_0 \}$. Defino $f(1) = x_1 \neq x_0$

    Notemos que si
    $X \neq \{ x_0 \}$ fuera finito 
    $\implies \underbrace{(X-\{x_0\})}_{\text{finito}}
    \cup \underbrace{\{ x_0 \}}_{\text{finito}} = X $ sería finito, lo cual
    es absurdo.
    ...
    $\exists \; x_2 \in X - \{x_0, x_1\}$. Defino $f(2) = x_2$
...
\begin{enumerate}
    ...
    \item $\exists \; f: \mathbb{N} \to A$ sobreyectiva $\implies A$ es
        numerable.
\end{enumerate}
...
\begin{enumerate}
    \item \begin{proof}\phantom{.}
            ...
            \nota{Notemos que $f: \mathbb{N} \to B$ es biyectiva.}%
            ...
            $A$ y $B$ numerables $\implies A \cup B$ numerables.
...
\begin{itemize}
    \item $\# (0,1) = C$
        ...
        \implies (0,1) 
        \underbrace{\sim}_{\text{Teorema } \ref{teo:inf-nonum-cjto-num}} 
        [0,1]\]
        \end{proof}
    ...
    \item $\# (-10, 10) = C$
        ...
        \begin{proof}\phantom{.}
        Veamos que se puede generalizar como $\# (a, b) = C$, con $a, b$ 
        números (ninguno es $\infty$).
    ...
    \item $\# \mathbb{R} = C$
        \begin{proof}\phantom{.}
            ...
            Por el item anterior, sabemos que $\# (a,b) = C$, entonces:
...
\begin{proof} Buena definición.
    ...
    \begin{enumerate}
        \item Sea $\underbrace{\widetilde{X} \sim X}_{\# \widetilde{X} = a}$.
            ...
            Sea $\underbrace{\widetilde{Y} \sim Y}_{\# \widetilde{Y} = b}$ tal que
            $\widetilde{X} \cap \widetilde{Y} = \varnothing$

            Quiero ver que $\# (\widetilde{X} \cup \widetilde{Y}) = \# (X \cup Y)$

            \begin{align*}
                \widetilde{X} \sim X & \implies \exists \; f: \widetilde{X} \to X
                \text{ biyectiva}\\
                \widetilde{Y} \sim Y & \implies \exists \; g: \widetilde{Y} \to Y
                \text{ biyectiva}\\
            \end{align*}
...
\begin{proof}\phantom{.}
    ...
    \begin{itemize}
        \item Iny.) $F(A) = F(B) \implies \Chi_A = \Chi_B 
...
\begin{proof}\phantom{.}
    ...
    Sean $U = [0,1]$, $\# U = C$, 
    $T = {\{0,1\}}^{\mathbb{N}} 
    = \{f:\mathbb{N} \to \{0,1\}/f \text{ es función}\}$, $\# T=2^{\aleph_0}$
    ...
    Conclusión, $F$ es biyectiva y, entonces, $\# (T-T_0) = \# \widetilde{U}$.
    Como $\#(T-T_0) = 2^{\aleph_0}$ y $\# \widetilde{U} = C$, queda probado que
...
\begin{align*}
    ...
    &\implies \boxed{{\aleph_0}^{\aleph_0} = C}
\end{align*}
